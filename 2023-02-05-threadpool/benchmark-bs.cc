...
public:
  ...
  void push_loop(T1 first_index_, T2 index_after_last_, F &&loop,
                 size_t num_blocks = 0) {
   ...
   * submitting each block separately to the queue. The user must use
   * wait_for_tasks() or some other method to ensure that the loop finishes
   * executing, otherwise bad things will happen. This overload is used for the
...
static void ThreadPoolTest(std::size_t size) {
  auto pool = BS::thread_pool_light{std::thread::hardware_concurrency()};

  std::vector<std::future<int>> result;
  result.reserve(size);
  for (std::size_t i = 0; i < size; ++i) {
    result.emplace_back(pool.submit(Worker, 1000000));
  }
